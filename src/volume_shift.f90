...
contains
  ...
  function initVolumeShift(nc,comp,shiftId,eos) result (volumeShiftId)
    ...
    type (gendata), dimension(nc), intent(inout) :: comp !< Component data
    ...
    if (trim(shiftId) == 'Peneloux') then
      do i=1,nc
        if (comp(i)%zra > 0.0) then
          if (trim(eos) == 'SRK') then
            comp(i)%ci = 0.40768*(Rgas*comp(i)%tc/comp(i)%pc)*&
                 (0.29441 - comp(i)%zra)
          else if (trim(eos) == 'PR') then
            comp(i)%ci = 0.50033*(Rgas*comp(i)%tc/comp(i)%pc)*&
                 (0.25969 - comp(i)%zra)
          else
            call stoperror('Wrong EOS for Peneloux volume shift')
          endif
        else
          print *, "NB: no volume shift parameter available for ", comp(i)%ident
  ...
  subroutine volumeShiftEnthalpy(nc,comp,volumeShiftId,T,P,Z,phase,h,dhdt,dhdp,dhdz)
    ...
    implicit none
    integer, intent(in) :: nc !< Number of components
    type (gendata), dimension(nc), intent(in) :: comp !< Component data
  ...
  subroutine volumeShiftFugacity(nc,comp,volumeShiftId,T,P,Z,phase,lnfug,dlnfdt,dlnfdp,dlnfdz)
    ...
    if (volumeShiftId == PENELOUX) then
      do j=1,nc
        lnfug(j) = lnfug(j) - comp(j)%ci*P/(Rgas*T)
      enddo
      if (present(dlnfdp)) then
        do j=1,nc
          dlnfdp(j) = dlnfdp(j) - comp(j)%ci/(Rgas*T)
        enddo
      endif
      if (present(dlnfdt)) then
        do j=1,nc
          dlnfdt(j) = dlnfdt(j) + P*comp(j)%ci/(Rgas*T**2)
        enddo
      endif
    endif
  ...
  subroutine volumeShiftZfac(nc,comp,volumeShiftId,t,p,z,phase,Zfac,dZdt,dZdp,dZdz)
    ...
    if (volumeShiftId == PENELOUX) then
      ...
      if (present(dzdz)) then
        do i=1,nc
          dzdz(i) = dzdz(i) - (comp(i)%ci - c/n)*P/(n*Rgas*T)
        enddo
      endif
    endif
  ...
  subroutine redefine_volume_shift(nc,j,comp,vLcurrent,vLexp)
    ...
    real :: v0
    !
    v0 = vLcurrent + comp(j)%ci  ! Volume without correction (m3/kmol)
    comp(j)%ci = v0 - vLexp  ! Updated volume-shift (m3/kmol)
