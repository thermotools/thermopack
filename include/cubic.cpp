...
void Cubic::init(const std::string& comps, const std::string& eos, const std::string& mixing,
				 const std::string& alpha, const std::string& parameter_reference, bool volume_shift) {
	typedef void (*init_func)(const char*, const char*, const char*, const char*, const char*, int*, int, int, int, int, int);
	init_func init_cubic = (init_func)dlsym(tp_handle, "__eoslibinit_MOD_init_cubic");
	...
	init_cubic(comps.c_str(), eos.c_str(), mixing.c_str(), alpha.c_str(), parameter_reference.c_str(),
			   &vol_shift_int, comps.size(), eos.size(), mixing.size(), alpha.size(), parameter_reference.size());
}
...
void Cubic::init_pseudo(const std::string& comps, const std::vector<double>& Tclist, const std::vector<double>& Pclist, const std::vector<double>& acflist, const std::vector<double>& Mwlist, const std::string& mixing, const std::string& alpha) {
	typedef void (*init_pseudo_func)(const char*, const double*, const double*, const double*, const double*, const char*, const char*);
	init_pseudo_func init_pseudo = (init_pseudo_func)dlsym(tp_handle, "__eoslibinit_MOD_init_cubic_pseudo");

	if (!init_pseudo) {
		throw std::runtime_error("Could not load init_cubic_pseudo from thermopack.");
	}

	init_pseudo(comps.c_str(), Tclist.data(), Pclist.data(), acflist.data(), Mwlist.data(), mixing.c_str(), alpha.c_str());
}
