...
module pc_saft_parameters
  ...
  type(pc_saft_data), parameter :: PCcx18 = pc_saft_data(eosPC_SAFT,"H2O", &
       1.50, 2.6273E-10, 180.3, 1804.22*Rgas, 0.0942, assoc_scheme_4C,1)

  type(pc_saft_data), parameter :: PCcx19 = pc_saft_data(eosPC_SAFT,"H2O", &
       1.0656, 3.0007E-10, 366.51, 2500.7*Rgas, 0.034868,  assoc_scheme_2B,2) ! [1]

  type(pc_saft_data), parameter :: PCcx20 = pc_saft_data(eosPC_SAFT,"CO2", &
       2.0729, 2.7852E-10, 169.21, 0.0, 0.0, no_assoc,1) ! [1]
  ! [10]
  type(pc_saft_data), parameter :: PCcx21 = pc_saft_data(eosPC_SAFT,"CO2", &
       2.58239e+00 ,2.56106e-10 ,1.51691e+02 ,0.0 ,0.0,no_assoc,2)
  ...
  type(pc_saft_data), parameter :: PCcx42 = pc_saft_data(eosPC_SAFT,"MEOH", &
       2.1, 2.7998E-10, 197.23, 2535.0*Rgas, 0.8230*PI_6, assoc_scheme_2C,1)
  ...
  type(pc_saft_data), parameter :: PCcx43 = pc_saft_data(eosPC_SAFT,"ETOH", &
       2.3609, 3.1895E-10, 207.56, 2695.69*Rgas, 0.03270*PI_6, assoc_scheme_2C,1)

  type(pc_saft_data), parameter :: PCcx44 = pc_saft_data(eosPC_SAFT,"ETOH", &
       1.2309, 4.1057E-10, 316.91, 2811.02*Rgas, 0.00633*PI_6, assoc_scheme_2B,2)
  ...
  type(pc_saft_data), parameter :: PCcx45 = pc_saft_data(eosPC_SAFT,"ETOH", &
       2.3827, 3.1771E-10, 198.24, 2653.4*Rgas, 0.032384, assoc_scheme_2B,3)
  ...
  type(pc_saft_data), parameter :: PCcx46 = pc_saft_data(eosPC_SAFT,"PROP1OL", &
       2.9537, 3.2473E-10, 226.36, 2448.02*Rgas, 0.02280*PI_6, assoc_scheme_2C,1)
  ...
  type(pc_saft_data), parameter :: PCcx47 = pc_saft_data(eosPC_SAFT,"BUT1OL", &
       2.9614, 3.5065E-10, 253.29, 2601.00*Rgas, 0.00740*PI_6, assoc_scheme_2C,1)
  ...
  type(pc_saft_data), parameter :: PCcx48 = pc_saft_data(eosPC_SAFT,"PENT1OL", &
       3.1488, 3.6350E-10, 261.96, 2555.34*Rgas, 0.00750*PI_6, assoc_scheme_2C,1)
  ...
  type(pc_saft_data), parameter :: PCcx49 = pc_saft_data(eosPC_SAFT,"HEX1OL", &
       3.1542, 3.8188E-10, 277.24, 2946.10*Rgas, 0.00250*PI_6, assoc_scheme_2C,1)
  ...
  type(pc_saft_data), parameter :: PCcx50 = pc_saft_data(eosPC_SAFT,"PROP1OL", &
       2.9997, 3.2522E-10, 233.40, 2276.8*Rgas, 0.015268, assoc_scheme_2B,2)
  ...
  type(pc_saft_data), parameter :: PCcx51 = pc_saft_data(eosPC_SAFT,"BUT1OL", &
       2.7515, 3.6189E-10, 259.59, 2544.6*Rgas, 0.006692, assoc_scheme_2B,2)
  ...
  type(pc_saft_data), parameter :: PCcx52 = pc_saft_data(eosPC_SAFT,"PENT1OL", &
       3.6260, 3.4508E-10, 247.28, 2252.1*Rgas, 0.010319, assoc_scheme_2B,2)
  ! Gross and Sadowski 2002, Ind. Eng. Chem. Res. 41 (DeltaP,Deltav)=(0.58,0.71)
  ...
  type(PCkijdata), dimension(PCmaxkij), parameter :: PCkijdb = (/ &
       PCkijdata(eosPC_SAFT,"H2S","C1",1,0.0425), &
       PCkijdata(eosPC_SAFT,"H2S","C2",1,0.072), &
       PCkijdata(eosPC_SAFT,"H2S","C3",1,0.069), &
       PCkijdata(eosPC_SAFT,"H2S","NC4",1,0.067), &
       PCkijdata(eosPC_SAFT,"H2S","NC5",1,0.073), &
       PCkijdata(eosPC_SAFT,"H2S","NC6",1,0.073), &
       PCkijdata(eosPC_SAFT,"H2S","NC7",1,0.078), &
       PCkijdata(eosPC_SAFT,"H2S","NC9",1,0.086), &
       PCkijdata(eosPC_SAFT,"H2S","NC10",1,0.077), &
       PCkijdata(eosPC_SAFT,"H2S","IC4",1,0.060), &
       PCkijdata(eosPC_SAFT,"H2S","IC5",1,0.076), &
       PCkijdata(eosPC_SAFT,"H2S","CYCLOHEX",1,0.082), &
       PCkijdata(eosPC_SAFT,"H2S","NC8",1,0.0), &
       PCkijdata(eosPC_SAFT,"CO2","H2S",1,0.0223), &
       PCkijdata(eosPC_SAFT,"CO2","BENZENE",1,0.025), &
       PCkijdata(eosPC_SAFT,"CO2","TOLU",1,0.026), &
       PCkijdata(eosPC_SAFT,"CO2","C1",1,0.0650), & ! Value from Gross and Sadowski 2001.
       PCkijdata(eosPC_SAFT,"CO2","C2",1,0.102), &
       PCkijdata(eosPC_SAFT,"CO2","C3",1,0.0107), &
       PCkijdata(eosPC_SAFT,"CO2","NC4",1,0.109), &
       PCkijdata(eosPC_SAFT,"CO2","NC5",1,0.120), &
       PCkijdata(eosPC_SAFT,"CO2","NC6",1,0.123), &
       PCkijdata(eosPC_SAFT,"CO2","NC7",1,0.115), &
       PCkijdata(eosPC_SAFT,"CO2","NC8",1,0.132), &
       PCkijdata(eosPC_SAFT,"CO2","NC9",1,0.122), &
       PCkijdata(eosPC_SAFT,"CO2","NC10",1,0.133), &
       PCkijdata(eosPC_SAFT,"CO2","IC4",1,0.112), &
       PCkijdata(eosPC_SAFT,"CO2","IC5",1,0.116), &
       PCkijdata(eosPC_SAFT,"CO2","CYCLOHEX",1,0.125), &
       PCkijdata(eosPC_SAFT,"CO2","BENZENE",1,0.087), &
       PCkijdata(eosPC_SAFT,"CO2","TOLU",1,0.108) &
       /)
...
contains
  ...
  function getPCdataIdx(eosidx,compName,setno) result(idx)
    ...
    found = .false.
    idx = 1
    do while (idx <= nPCmodels .and. .not. found)
      if ((eosidx==PCarray(idx)%eosidx) .and. &
           str_eq(compName, PCarray(idx)%compName) .and. &
           setno==PCArray(idx)%setno) then
        found = .true.
      else
        idx = idx + 1
      endif
  ...
  function getPCkij (eosidx,uid1,uid2,setno) result(kijvalue)
    use stringmod, only: str_eq
    ...
    do idx = 1,PCmaxkij
       ...
       if ( eosidx==PCkijdb(idx)%eosidx .and. (match_11_22 .or. match_12_21) &
            .and. PCkijdb(idx)%setno==setno_loc) then
  ...
  subroutine getPcSaftKij_allComps(nc,comp,eosidx,kij)
    ...
    do ic=1,nc
       do jc=ic+1,nc
          kij(ic,jc) = getPCkij(eosidx,comp(ic)%ident,comp(jc)%ident,setno=1)
          kij(jc,ic) = kij(ic,jc)
       end do
    end do
  ...
  subroutine getPcSaftPureParams_allComps(nc,comp,eosidx,setno,found,m,sigma,eps_depth_divk,eps,beta,scheme)
    use compdata, only: gendata
    ! Input
    integer, intent(in) :: nc
    type(gendata), intent(in) :: comp(nc)
    ...
    do ic=1,nc
       call getPcSaftpureParams_singleComp(comp(ic)%ident,eosidx,setno(ic),found(ic),m(ic),&
            sigma(ic),eps_depth_divk(ic),eps(ic),beta(ic),scheme(ic))
    end do

  end subroutine getPcSaftPureParams_allComps

  subroutine getPcSaftpureParams_singleComp(compName,eosidx,setno,found,m,sigma,eps_depth_divk,eps,beta,scheme)
    ! Input
    character(len=*), intent(in) :: compName
    integer, intent(in) :: eosidx,setno
    ! Output
    ...
    idx = getPCdataIdx(eosidx,compName,setno)
    if ( idx == 0 ) then
       found = .false.
       return
    end if
