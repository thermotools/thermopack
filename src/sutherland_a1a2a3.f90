module sutherland_a1a2a3
  !> The SAFT-VR Mie implementation of the perturbation terms a1,a2,a3
  !> for the Sutherland potential, defined by u(r) =
...
contains
  ...
  !> Calculate second-order perturbation term a2tilde=a2/eta
  subroutine calc_a2tilde_sutherland(x0,eta,eta_av,lambda,eps,alpha,a2,K_hs,chi)
    ! Input
    type(hyperdual), intent(in) :: x0     !< Reduced center-center hard sphere distance
    type(hyperdual), intent(in) :: eta    !< Packing fraction (-)
    type(hyperdual), intent(in) :: lambda !< Sutherland exponent (-)
  ...
  !> Calculate third-order perturbation term a1tilde=a1/eta
  ...
  !> Calculate isothermal hard-sphere compressibility factor for the correlation of a2
