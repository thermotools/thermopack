...
contains
  ...
  subroutine set_sutsum_parameters_from_Qmie(eos,nc, FHorder)
    ...
    ! Locals
    type(hyperdual) :: D, denum1, denum2, mi_inv, mj_inv, h_const_reduced_squared !< Potential parameters
    type(Sutherlandsum) :: sutsum
    integer :: i, j
  ...
  subroutine dhs_BH_Mie(T_r,mie,dhs)
    ...
    type(hyperdual), intent(out) :: dhs       ! hard-sphere diameter (m)
    ! Locals
    type(hyperdual) :: ap, nu, T14, C0, C1, C2, C3, C4, D, six, seven
    ...
    ap = 1.0/mie%alpha_x() - 1.0/mie_76%alpha_x()
