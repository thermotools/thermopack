module single_phase
  !---------------------------------------------------------------------
  ! Module which calculates important single-phase properties.
...
contains
  ...
  !---------------------------------------------------------------------- >
  !> This function contains the minimum of necessary properties needed by a new EoS
  !! in most calculations. Other properties can be derived from these.
  !! Numerical derivatives can also be calculated based on this routine. The following
  !! functions are needed: Zfac, Entropy, Enthalpy, Fugacity
  !!
  ...
  !!        Gibbs free energy is to be returned - calls cbGres   !! \retval Zfactor [-]
  !!
  !! \retval res The variable containg the wanted property [-]
  !!
  !! \author Oivind W
  function tpfunctions_necessary(nc,comp,cbeos,T,P,Z,phase,prop_string,gflag_opt) result (res)
    use tpcubic
    ...
    real, dimension(nc) :: res
    !

    if(str_eq(prop_string,'ZFAC')) then ! Zfactor
      res(1) = fork_Zfac_calculation(nc,comp,cbeos,T,P,Z,phase,gflag_opt)
    elseif(str_eq(prop_string,'ENTROPY')) then    ! Calculate the entropy
      res(1) = TP_CalcEntropy(nc,comp,cbeos,T,P,Z,phase,.false.,gflag_opt=gflag_opt)
    elseif(str_eq(prop_string,'ENTHALPY')) then ! Calculate the enthalpy
      res(1) = TP_CalcEnthalpy(nc,comp,cbeos,T,P,Z,phase,.false.,gflag_opt=gflag_opt)
    elseif(str_eq(prop_string,'FUGACITY')) then ! Calculate the fugacity
      call TP_lnfug(nc,comp,cbeos,phase,T,P,z,res,gflag_opt=gflag_opt)
    end if

  end function tpfunctions_necessary

  !---------------------------------------------------------------------- >
  !> This function calculates the numerical derivatives of the folloing thermodynamic
  !! properties: Zfac, Entropy, Enthalpy, Fugacity
  !!
  !! \param T The temperature [K]
  !! \param P The pressure [Pa]
  !! \param Z The mole fraction [-]
  ...
  !!        Gibbs free energy is to be returned - calls cbGres   !! \retval Zfactor [-]
  !!
  !! \retval d_res Numerical derivatives [-]
  !!
  !! \author Oivind W
  subroutine tp_numderiv(nc,comp,cbeos,T,P,Z,phase,prop_string,gflag_opt,dPropdt,dPropdp,dPropdz)
    use tpcubic
    use stringmod, only: str_eq
    use utilities, only: normalize
    use eosdata
    use compdata, only: gendata
    implicit none
    integer, intent(in) :: nc
    ...
    real, intent(in) :: T, P
    integer, intent(in) :: phase
    real, optional, intent(out) :: dPropdt,dPropdp
    real, optional, dimension(nc), intent(out) :: dPropdz
    character (len=*), intent(in) :: prop_string
    integer, optional :: gflag_opt

    ! Locals
    real :: Tp, Tm, Pp, Pm
    real, dimension(nc) :: Zp, Zm
    integer :: i
    real, dimension(nc) :: res_Tp, res_Tm, res_Pp, res_Pm, res_Zm, res_Zp

    if (.NOT. present(gflag_opt)) then
      gflag_opt=2
    end if
    !
    if(str_eq(prop_string,'ZFAC') .OR. str_eq(prop_string,'ENTROPY') .OR. &
         str_eq(prop_string,'ENTHALPY')) then !

      if(present(dPropdt)) then  ! Numerical temperature derivative
        Tp=T+delta_T
        Tm=T-delta_T
        res_Tp=tpfunctions_necessary(nc,comp,cbeos,Tp,P,Z,phase,prop_string,gflag_opt)
        res_Tm=tpfunctions_necessary(nc,comp,cbeos,Tm,P,Z,phase,prop_string,gflag_opt)
        dPropdt=(res_Tp(1)-res_Tm(1))/(2*delta_T)
      end if

      if(present(dPropdp)) then ! Numerical pressure derivative
        Pp=P+delta_P
        Pm=P-delta_P
        res_Pp=tpfunctions_necessary(nc,comp,cbeos,T,Pp,Z,phase,prop_string,gflag_opt)
        res_Pm=tpfunctions_necessary(nc,comp,cbeos,T,Pm,Z,phase,prop_string,gflag_opt)
        dPropdp=(res_Pp(1)-res_Pm(1))/(2*delta_P)
      end if

      if(present(dPropdz)) then ! Numerical composition derivative
        do i=1,nc
          ! Assign compositions
          Zp=Z
          Zm=Z
          ...
          res_Zp=tpfunctions_necessary(nc,comp,cbeos,T,P,Zp,phase,prop_string,gflag_opt)
          res_Zm=tpfunctions_necessary(nc,comp,cbeos,T,P,Zm,phase,prop_string,gflag_opt)
          dPropdz(i)=(res_Zp(1)-res_Zm(1))/(2*delta_Ni)
        end do
      end if
    end if

  end subroutine tp_numderiv
  ...
  !!
  !! \author Oivind W
  subroutine tp_numderiv_matrix(nc,comp,cbeos,T,P,Z,phase,prop_string,gflag_opt,&
       dPropdt,dPropdp,dPropdz,v)
    use tpcubic
    use stringmod, only: str_eq
    use utilities, only: normalize
    use eosdata
    ...
    real, intent(in) :: T, P
    integer, intent(in) :: phase
    real, optional, dimension(nc), intent(out) :: dPropdt,dPropdp
    real, optional, dimension(nc,nc), intent(out) :: dPropdz
    character (len=*), intent(in) :: prop_string
    integer, optional, intent(in) :: gflag_opt
    ...
    real, dimension(nc) :: res_Tp, res_Tm, res_Pp, res_Pm, res_Zm, res_Zp
    !
    if(str_eq(prop_string,'FUGACITY')) then ! The fugacity (has nc components)
      ...
      if(present(dPropdt)) then  ! Numerical temperature derivative
        ...
        dPropdt=(res_Tp-res_Tm)/(2*delta_T) ! (dfi/dT)
      end if
      ...
      if(present(dPropdp)) then ! Numerical pressure derivative
        ...
        res_Pp=tpfunctions_necessary(nc,comp,cbeos,T,Pp,Z,phase,prop_string,gflag_opt)
        res_Pm=tpfunctions_necessary(nc,comp,cbeos,T,Pm,Z,phase,prop_string,gflag_opt)
        dPropdp=(res_Pp-res_Pm)/(2*delta_P) ! (dfi/dP)
      end if
      ...
      if(present(dPropdz)) then ! Numerical composition derivative
        ...
        do i=1,nc
          ...
          ! Fugacity derivative: dfj/dNi
          dPropdz(:,i)=(res_Zp-res_Zm)/(2*delta_Ni)
        end do
      end if  ! Composition derivative
    end if

  end subroutine tp_numderiv_matrix
